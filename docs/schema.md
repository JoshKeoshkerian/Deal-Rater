...
| `home_returned`, `home_usable` | The home search's own composition. A page that came back mostly same-model means Facebook still had inventory and merely chose which trims to show; a page padded out with other models means the radius is out of them. **This is the signal that gates the trim query**, cut at 0.50 (`widen.MIN_HOME_MODEL_FRACTION_FOR_TRIM_QUERY`); still recorded so the threshold can be re-checked. |
...
| `trim_query_skipped` | Why it did not fire: `unscoped_home_search`, `trim_target_already_met`, `market_exhausted`, `no_trim_level`. A gate that records only its hits cannot be evaluated, and `market_exhausted` is a threshold set from four observations. |
